...
class SimpleAgent:
    def __init__(self, model, actions, optimizer=Adam(learning_rate=1e-3), metrics: list = None):
        self._model = model
        self._policy = BoltzmannQPolicy()
        self._memory = SequentialMemory(limit=14000000, window_length=1)
        ...
        if metrics is None:
            metrics = ['mse']
        self._agent.compile(optimizer=optimizer, metrics=metrics)
