...
from ML_EVR.common.envs.combination_envs import EvoSortClsEnvSPT
...
class Runner:
    ...
    def reorder(scenarios: list, executor: EvrExecutor) -> Executor:
        scenarios.sort(key=lambda scenario: executor.get_max_time_if_scenario_is_added(scenario))
    ...
    def run(test_case: int = 1, real: bool = True):
        r1 = ClassificationRunner(env_class=EvoSortClsEnvSPT, test_set=test_case, real=real)
        r1.run()
        r1.tester.env.reset()
        start = time.time()
        r1.tester.test()
